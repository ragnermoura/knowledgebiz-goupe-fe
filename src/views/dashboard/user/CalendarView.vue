...
/* 
    remainingHours() {
      const selectedDate = new Date(this.selectedDate || null);
      const selectedDateStr = selectedDate?.toISOString().split("T")[0];

      // Filtra os eventos que ocorrem na data selecionada
      ...
      // Calcula o total de minutos dos eventos na data selecionada
      const totalMinutes = eventsOnSelectedDate?.reduce((total, event) => {
        const [hours, minutes] = event.time?.split(":");
        return total + parseInt(hours) * 60 + parseInt(minutes);
      }, 0);

      // Calcula as horas restantes das 8 horas
      const remainingMinutes = Math.max(8 * 60 - totalMinutes, 0);
...
const selectedDate = new Date(defaultDate);
  ...
  return horaFormatada;
            ...
            const percentage = ( this.timeSpent /  100 ) * 8; // Calcula a porcentagem
